...
def plot_percent_sick_over_time(tot_eval_data):
    agent_names = list(tot_eval_data.keys())
    aggregated_tot_ep_rews = [tot_eval_data[name]['tot_percent_sick_over_time'] for name in agent_names]

    timesteps = list(range(len(aggregated_tot_ep_rews[0][0])))
    num_eps = len(aggregated_tot_ep_rews[0])

    plt.title(f'Average Percent Sick Over Time')
    plt.xlabel('Timestep')
